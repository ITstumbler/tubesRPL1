...
function AdminEditChef() {
    const location = useLocation();
    const [chefData, setChefData] = useState({nama_koki: "Memuat Data...", tanggal_diterima: "", status: "Aktif"});
    ...
    useEffect(() => {
        ...
        if(!location.state.id_koki) navigate("/admin_chef_list", {state: {token: location.state.token, email: location.state.email, role: location.state.role}});
        fetch(backendUrl + `/viewentry?table=3&id=`+location.state.id_koki)
        .then(res => res.json())
        .then(jsondata => {setChefData(jsondata)});
    }, [location]);
    ...
    const submitEditedChefData = (e) => {
        e.preventDefault();
        if(chefData.nama_koki === undefined
        || chefData.tanggal_diterima  === undefined
        || chefData.status  === undefined) {
            setCustomStatusMessage({message: "Mohon mengisi semua kolom", error: 1});
        }
        else {
            if(userToken === "") {
                setCustomStatusMessage({message: "Harus login untuk mengedit data", error: 1});
            }
            else {
                const payload = {
                    method: "POST",
                ...
                fetch(backendUrl + "/editchefdata?id="+location.state.id_koki, payload)
                .then(res => res.json())
    ...
    return(
        <>
        <div className="headertext">EDIT DATA KOKI
        ...
        <br></br>
        <form onSubmit={submitEditedChefData}>
