...
function AdminEditUser() {
    ...
    const [userData, setUserData] = useState({peran: location.state.peran_user, id_karyawan: "1", email: "", status: "Aktif", password: ""});
    ...
    useEffect(() => {
        ...
        if(!location.state.id_user) navigate("/admin_user_list", {state: {token: location.state.token, email: location.state.email, role: location.state.role}});
        fetch(backendUrl + `/viewentry?table=5&id=`+location.state.id_user+`&peran=`+location.state.peran_user)
        .then(res => res.json())
    ...
    const submitEditedUserData = (e) => {
        ...
        || userData.status  === "") {
            ...
            setCustomStatusMessage({message: "Password harus diisi ulang", error: 1});
        }
        else {
            if(userToken === "") {
                ...
                fetch(backendUrl + "/edituserdata?id="+location.state.id_user, payload)
                .then(res => res.json())
    ...
    return(
        <>
        <div className="headertext">EDIT DATA USER
        ...
        <br></br>
        <form onSubmit={submitEditedUserData}>
