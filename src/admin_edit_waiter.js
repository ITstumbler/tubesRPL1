...
function AdminEditWaiter() {
    const location = useLocation();
    const [waiterData, setWaiterData] = useState({nama_pelayan: "Memuat Data...", tanggal_diterima: "", status: "Aktif"});
    ...
    useEffect(() => {
        ...
        if(!location.state.id_pelayan) navigate("/admin_waiter_list", {state: {token: location.state.token, email: location.state.email, role: location.state.role}});
        fetch(backendUrl + `/viewentry?table=2&id=`+location.state.id_pelayan)
        .then(res => res.json())
        .then(jsondata => {setWaiterData(jsondata)});
    }, [location]);
    ...
    const submitEditedWaiterData = (e) => {
        e.preventDefault();
        if(waiterData.nama_pelayan === undefined
        || waiterData.tanggal_diterima  === undefined
        || waiterData.status  === undefined) {
            ...
            if(userToken === "") {
                ...
                fetch(backendUrl + "/editwaiterdata?id="+location.state.id_pelayan, payload)
                .then(res => res.json())
    ...
    return(
        <>
        <div className="headertext">EDIT DATA PELAYAN
        ...
        <br></br>
        <form onSubmit={submitEditedWaiterData}>
...
export default AdminEditWaiter;